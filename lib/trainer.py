...
from .image_utils import quantize, save_result
...
class Trainer:
    def __init__(self, options):
        ...
        if self.options.model.type == "mssdiff":
    ...
    def evaluate_one(self, epoch):
        ...
        for i, sample in enumerate(
            ...
            if epoch % self.options.train.val_images_interval == 0:
                ...
                statistics["mse diff"] = torch.nn.functional.mse_loss(image, gen).mean().item()
    ...
    @torch.no_grad()
    def test_one(self, epoch=None, save_images=True):
        if epoch is None:
        ...
        for sample in tqdm(
            ...
            if save_images:
                for idx in range(idx_base, idx_base + image.shape[0]):
                    ...
                    save_result(gen_diff[idx - idx_base], save_path)
